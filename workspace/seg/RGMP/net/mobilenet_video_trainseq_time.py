...
#               activation_fn=None,
  ...
  def bridge_conv(inputs, num_outputs, scope=None):
    with tf.variable_scope(scope):
      net = output_conv_unit(inputs,num_outputs)
...
def GC(net, scope='GC',kw=7,kh=7,is_training=True,):
  ...
  with tf.variable_scope(scope):
    ...
    netl = conv_1_7(net, 32)
    netl = conv_7_1(netl, 8)

    netr = conv_7_1(net, 32)
    netr = conv_1_7(netr, 8)
...
def mobilenet(inputs,
  ...
  for cnt in range(FLAGS.train_length):
    if cnt > 0:
      reuse = True
    with tf.variable_scope(scope, reuse=reuse) as scope1:
      if cnt == 0:
        if FLAGS.train_length == 1:
          if FLAGS.train_list == True:
  ...
  if FLAGS.train_list == False:
    return Logits_list[0],end_points
