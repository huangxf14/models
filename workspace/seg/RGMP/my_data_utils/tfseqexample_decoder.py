...
class TFSeqExampleDecoder(data_decoder.DataDecoder):
  ...
  def __init__(self, keys_to_context_features, keys_to_sequence_features, items_to_handlers, items_to_handlers_list):
    ...
    self._items_to_handlers_list = items_to_handlers_list

  def list_items(self):
    """See base class."""
    return list(self._items_to_handlers.keys())+list(self._items_to_handlers_list.keys())
  ...
  def decode(self, serialized_example, items=None):
    ...
    """
    context_features, sequence_features = parsing_ops.parse_single_sequence_example(serialized_example,
                                               self._keys_to_context_features,
                                               self._keys_to_sequence_features)

    # Reshape non-sparse elements just once, adding the reshape ops in
    # deterministic order.

    for k in sorted(self._keys_to_context_features):
      v = self._keys_to_context_features[k]
      if isinstance(v, parsing_ops.FixedLenFeature):
        context_features[k] = array_ops.reshape(context_features[k], v.shape)

    # for k in sorted(self._keys_to_sequence_features):
    #   v = self._keys_to_sequence_features[k]
    #   if isinstance(v, parsing_ops.FixedLenSequenceFeature):
    #     sequence_features[k] = array_ops.reshape(sequence_features[k], v.shape)

    if not items:
      items = self._items_to_handlers.keys() + self._items_to_handlers_list.keys()

    outputs = []
    for item in items:
      if item in self._items_to_handlers.keys():
        handler = self._items_to_handlers[item]
        keys_to_tensors = {key: context_features[key] for key in handler.keys}
        outputs.append(handler.tensors_to_item(keys_to_tensors))
      else:
        handler = self._items_to_handlers_list[item]
        keys_to_tensors = {key: context_features[key] if key in context_features.keys() else sequence_features[key] for key in handler.keys}
        outputs.append(handler.tensors_to_item(keys_to_tensors))
