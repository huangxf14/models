...
def _images_list_feature(values):
  """Returns a TF-FeatureList of images.
  ...
  Returns:
    A TF-FeatureList.
  """
  return tf.train.FeatureList(feature=[_bytes_list_feature(value) for value in values])

  

def image_seg_to_tfexample(image_data, filename, height, width, seg_data, lastmask_data, firstimage_data, firstmask_data):
...
def video_seg_to_tfexample(image_data, filename, height, width, seg_data, image_num):
  """Converts one image/segmentation pair to tf example.
  ...
  return tf.train.SequenceExample(context=tf.train.Features(feature={
      ...
      'image/segmentation/class/format': _bytes_list_feature(
          FLAGS.label_format),
      'image/num': _int64_list_feature(image_num),
  }),
  feature_lists=tf.train.FeatureLists(feature_list={
    'image/encoded': _images_list_feature(image_data),
    'image/segmentation/class/encoded': (
          _images_list_feature(seg_data)),
    }),
